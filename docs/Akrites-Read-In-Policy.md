...
|---|---|---|---|---|
| **Tier 0** | Reporter / Finder and the SIRT (plus automated intake) | Originate the report; own the fix and the final disclosure decision | Full case detail; TLP:RED at intake | Earliest; longest duration |
| **Tier 1** | Subject-matter experts (Akrites members) | Human & LLM-assisted analysis to produce, and test the patch | Case material needed to build and test the fix; TLP:AMBER+STRICT | When analysis/testing begins |
| **Tier 1.5** | Upstream Maintainer/Security Team (Non-Akrites members) | Human-brokered collaboration to analyze, produce, and test the patch | Case material needed to build and test the fix; TLP:AMBER+STRICT | When analysis/testing is ready to share with upstream |
