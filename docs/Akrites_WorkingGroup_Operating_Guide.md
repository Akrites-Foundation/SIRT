...
Additional terms and definitions can be found in the [Akrites SIRT & CVD Glossary
...
- **Named parties only.** Discuss a case only with the people read into *that case*. If someone isn't read in, you cannot discuss the Case with them. They don't exist as far as the Case is concerned. Follow the [SIRT Read-in Guidelines](https://github.com/Akrites-Foundation/SIRT/blob/main/docs/SIRT%20Read-in%20Guidelines.md) to have someone read in, enabling you to discuss the Case with them.
...
- **No sensitive artifacts in chat or via unencrypted email.** Never put exploit code, PoCs, or patches in Slack. Those belong in the private repository using Akrites infrastructure.
...
0. **Notify the SIRT** Submit the following information to [SIRT@Akrites.dev](mailto:SIRT@Akrites.dev?subject=New-Akrites-WG) in order to establish a new WG.
* **Name of WG:**
* **Scope of WG:** 
...
If both are yes, send the [SIRT](mailto:SIRT@Akrites.dev?subject=New-Akrites-WG-Read-In) the person's name, org, and which of those roles they'll perform. The SIRT verifies and grants access.
